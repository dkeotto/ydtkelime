...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('user-joined', ({ username, socketId, isHost, avatar, studied, known }) => {
      console.log('Kullanıcı katıldı:', username, 'isHost:', isHost);
      setUsers(prev => {
          ...
          <div className="competitors">
            {users.length === 0 ? (
              ...
              users
                ...
                .map((user, index) => (
                  <div key={user.username} className={`competitor ${user.username === username ? 'me' : ''}`}>
                    <span className="rank">#{index + 1}</span>
                    <span className="avatar">{user.avatar || '👤'}</span>
                    <span className="name">{user.username} {user.username === username ? '(Sen)' : ''}</span>
                    <span className="score-detail">
                      <span className="studied" title="Toplam">📚 {user.studied || 0}</span>
                      <span className="correct" title="Doğru">✓ {user.known || 0}</span>
                      <span className="wrong" title="Yanlış">✗ {(user.studied || 0) - (user.known || 0)}</span>
                    </span>
                  </div>
                ))
            )}
